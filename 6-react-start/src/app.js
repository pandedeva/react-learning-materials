...
function App() {
  ...
  function saveTodoHandler(event) {
    ...
    if (edit.id) {
      ...
      // lalu akses berdasarkan index, lalu set dengan object yang baru
      updatedTodos[editTodoIndex] = updatedTodo;

      // memanggil setTodos, agar tampil
  ...
  function removeTodoHandler(todoId) {
    const filteredTodos = todos.filter((todo) => {
      ...
      // jika todo.id tidak sama dengan todoId, tampilkan di array baru
    });

    // hasil filter dari fungsi diatas, ditampilkan di setTodos ini dan menampilkan array baru
    setTodos(filteredTodos);

    // kalau ada id dalam object edit, maka panggil function
          ...
          {todos.map((todo) => {
            return (
              <li key={todo.id}>
                ...
                {/* untuk mengirim argumen bisa menggunakan bind*/}
              </li>
            );
          })}
        </ul>
